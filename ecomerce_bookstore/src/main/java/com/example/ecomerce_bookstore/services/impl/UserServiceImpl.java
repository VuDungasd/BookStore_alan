...
import com.example.ecomerce_bookstore.entities.*;
...
import com.example.ecomerce_bookstore.payloads.*;
...
//            List<BookDTO> books = user.getCart().getCartItems().stream()
//                    .map(item -> modelMapper.map(item.getBook(), BookDTO.class)).collect(Collectors.toList());

//            dto.setCart(cart);
//
//            dto.getCart().setBooks(books);
...
//        userDTO.getCart().setBooks(books);
